...
export const useStore = defineStore("main", {
  ...
  getters: {
    guestsCount: (state) => {
      return state.adultsCount + state.childrenCount;
    },
    locations: (state) => {
      const locations = [];
    ...
    searchForCurrentFilters: (state) => {
      if (state.selectedLocation === "Select Location")
        return state.stays.filter(
          (stay) => stay.maxGuests >= state.guestsCount
        );
      const loc = state.selectedLocation.split(", ");
      const [city] = loc;
      return state.stays.filter(
        (stay) => stay.city === city && stay.maxGuests >= state.guestsCount
      );
    },
  },
});
